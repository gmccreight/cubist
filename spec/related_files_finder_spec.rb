...
describe Cubist::RelatedFilesFinder do
  ...
  describe "all" do
    it "should get the top ranked files by all connections between all files" do
      ...
      expect(files[:all].first(2).map{|x| x[:file]}).to eq(["app/models/product.rb", "app/models/item.rb"])
      ...
      expect(files[:all].first(2).map{|x| x[:file]}).to eq(["app/models/item.rb", "app/models/product.rb"])
  ...
  describe "per_file" do
    def expect_per_file_for(filename, expected_files)
      ...
      per_file = files[:per_file][filename].map{|x| x[:file]}
